...
import { z } from "zod";
import { emptyInputSchema, getIncentiveGroupInputSchema, getIncentiveInputSchema, getPaymentInputSchema, getWalletConsolidatedBalanceInputSchema, getSaleInputSchema, listGroupParticipantsInputSchema, listIncentiveGroupsInputSchema, listIncentivesInputSchema, listMyCompaniesInputSchema, listPaymentsInputSchema, listSalesInputSchema, whoamiInputSchema, } from "../schemas/honorfyMcpInputs.js";
...
function pickCompanyId(candidate) {
...
function firstCompanyIdFromListResponse(payload) {
...
async function resolveCompanyId(config, tokenProvider, companyId, required, cache) {
    const resolved = companyId ?? config.HONORFY_DEFAULT_COMPANY_ID;
    if (resolved)
        return resolved;
    if (!required)
        return undefined;
    if (cache.inferredCompanyId)
        return cache.inferredCompanyId;
    const ctx = trpcContextFromConfig(config, tokenProvider, undefined);
    const companies = await trpcGetJson("company.listMyCompanies", {}, ctx);
    ...
    throw new HonorfyMcpError({
        ...
        message: "companyId não informado e não foi possível inferir via company.listMyCompanies.",
        retryable: false,
    });
}
async function findInPaginated(load, match, opts) {
...
export function registerHonorfyTools(server, deps) {
    const { config, tokenProvider, rateLimiter } = deps;
    const companyIdCache = {};
    const run = (toolName, handler) => async (args) => {
        ...
        try {
            ...
            logStructured({
                ...
                durationMs: Date.now() - started,
                message: "tool_error",
                extra: err instanceof HonorfyMcpError
    ...
    server.registerTool("honorfy_list_companies", {
        title: "Listar empresas (v1)",
        description: "Lista empresas do usuário autenticado (`company.listMyCompanies`). Não envia x-company-id.",
        inputSchema: listMyCompaniesInputSchema,
    }, run("honorfy_list_companies", async (_args) => {
        const ctx = trpcContextFromConfig(config, tokenProvider, undefined);
        return trpcGetJson("company.listMyCompanies", {}, ctx);
    }));
    server.registerTool("honorfy_list_sales", {
        title: "Listar vendas (v1)",
        description: "Lista vendas (`sale.list`). Usa companyId informado, default ou tenta inferir via `company.listMyCompanies`.",
        inputSchema: listSalesInputSchema,
    }, run("honorfy_list_sales", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
    ...
    server.registerTool("honorfy_get_sale", {
        title: "Obter venda por ID (v1)",
        description: "Busca uma venda pelo id percorrendo páginas de `sale.list` (a API não expõe get único público).",
        inputSchema: getSaleInputSchema,
    }, run("honorfy_get_sale", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        const sale = await findInPaginated(async (page, limit) => trpcGetJson("sale.list", { page, limit }, ctx), (item) => String(item.id) === String(args.saleId));
        if (!sale) {
            throw new HonorfyMcpError({
                code: "NOT_FOUND",
    ...
    server.registerTool("honorfy_list_payments", {
        title: "Listar transações da carteira (v1)",
        description: "Lista transações (`payment.wallet.listTransactions`). Usa companyId informado, default ou inferido.",
        inputSchema: listPaymentsInputSchema,
    }, run("honorfy_list_payments", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const { companyId: _c, ...rest } = args;
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        return trpcGetJson("payment.wallet.listTransactions", rest, ctx);
    }));
    server.registerTool("honorfy_get_wallet_consolidated_balance", {
        title: "Obter saldo consolidado da carteira (v1)",
        description: "Retorna o consolidado da carteira (`payment.wallet.getConsolidatedBalance`), em centavos e por status.",
        inputSchema: getWalletConsolidatedBalanceInputSchema,
    }, run("honorfy_get_wallet_consolidated_balance", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        return trpcGetJson("payment.wallet.getConsolidatedBalance", {}, ctx);
    }));
    server.registerTool("honorfy_get_payment", {
    ...
    }, run("honorfy_get_payment", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        const tx = await findInPaginated(async (page, limit) => trpcGetJson("payment.wallet.listTransactions", { page, limit }, ctx), (item) => String(item.id) === String(args.transactionId));
        if (!tx) {
            throw new HonorfyMcpError({
                code: "NOT_FOUND",
    ...
    }, run("honorfy_list_incentive_groups", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const { companyId: _c, ...rest } = args;
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        return trpcGetJson("gamification.listIncentiveGroups", rest, ctx);
    }));
    server.registerTool("honorfy_get_incentive_group", {
    ...
    }, run("honorfy_get_incentive_group", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        const group = await findInPaginated(async (page, limit) => trpcGetJson("gamification.listIncentiveGroups", { page, limit }, ctx), (item) => String(item.id) === String(args.groupId));
        if (!group) {
            throw new HonorfyMcpError({
                code: "NOT_FOUND",
    ...
    server.registerTool("honorfy_list_group_participants", {
    ...
    }, run("honorfy_list_group_participants", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const { companyId: _c, ...rest } = args;
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        return trpcGetJson("gamification.listParticipants", rest, ctx);
    }));
    server.registerTool("honorfy_whoami", {
        title: "Quem sou (v1)",
        description: "Perfil do usuário autenticado (`user.getProfile`); usa companyId informado, default ou inferido.",
        inputSchema: whoamiInputSchema,
    }, run("honorfy_whoami", async (args) => {
        const companyId = await resolveCompanyId(config, tokenProvider, args.companyId, true, companyIdCache);
        const ctx = trpcContextFromConfig(config, tokenProvider, companyId);
        return trpcGetJson("user.getProfile", {}, ctx);
    }));
    server.registerTool("honorfy_capabilities", {
    ...
    }, run("honorfy_capabilities", async (_args) => ({
...
//# sourceMappingURL=registerHonorfyTools.js.map