...
describe("trpcGetJson", () => {
  ...
  it("retorna result.data em sucesso", async () => {
    ...
    const ctx = trpcContextFromConfig(
      baseConfig,
      new EnvUserTokenProvider("jwt"),
      "company1",
    );
    const data = await trpcGetJson<{ ok: boolean }>(
      "sale.list",
      { page: 1, limit: 10 },
      ctx,
    );
  ...
  it("lança HonorfyMcpError em HTTP de erro", async () => {
    ...
    await expect(
      trpcGetJson("company.listMyCompanies", {}, ctx),
    ).rejects.toBeInstanceOf(HonorfyMcpError);
  });
});
